...
class Tank_auth
{
	 ...
	 * @return	array
	 */
	function create_user($username, $email, $password, $email_activation,$apellido1){
	...
	function create_users($data,$email_activation){
		
		if ((strlen($data['username']) > 0) AND !$this->ci->users->is_username_available($data['username'])) {
			$this->error = array('username' => 'auth_username_in_use');

		} elseif (!$this->ci->users->is_email_available($data['email'])) {
			$this->error = array('email' => 'auth_email_in_use');
